    ...
    while(fscanf(file, "%s", next_word) == 1){
        ...
        if(total_length + num_words > column_size){
            ...
            while(words_to_print > 0){
                ...
                if(((need_spaces - spaces_printed) % words_to_print) * 2 >= words_to_print) // spreads spaces out along the line
                    ++spaces_to_print;
                spaces_printed += spaces_to_print;
                --words_to_print;
                chars_printed += fprintf(out, "%*c%s", spaces_to_print, ' ', word_print);//print char string
